...
def parse_pdf_text(text, norme=None):
    ...
    Si des valeurs sont manquantes mais que les données d'entrée sont présentes,
    Claude tente de les estimer (clés avec suffixe _estime).
    ...
    if ANTHROPIC_API_KEY:
        try:
            prompt = f"""Tu es un expert en réglementation thermique française (RT2012, RE2020).
Voici le texte extrait d'un document thermique (notice, synthèse, CCTP, étude STD).

## ÉTAPE 1 — EXTRACTION DIRECTE
Extrais les valeurs numériques présentes explicitement dans le texte :
- rt2012_bbio (Bbio)
- rt2012_cep (Cep en kWh ep/m².an)
- rt2012_tic (Tic en °C)
- rt2012_airtightness (étanchéité à l'air / perméabilité sous 4Pa en m³/h.m²)
- rt2012_enr (ENR)
- re2020_energy_efficiency (Cep,nr)
- re2020_thermal_comfort (DH degrés-heures)
- re2020_carbon_emissions (Ic énergie CO2)
- peb_espec, peb_ew, peb_u_mur, peb_u_toit, peb_u_plancher
- minergie_qh, minergie_qtot, minergie_n50
- sia380_qh
- cneb_ei, cneb_u_mur, cneb_u_toit, cneb_u_fenetre, cneb_infiltration
- lenoz_ep, lenoz_ew, lenoz_u_mur, lenoz_u_toit

## ÉTAPE 2 — ESTIMATION (uniquement pour RT2012/RE2020)
Si certaines valeurs RT2012/RE2020 sont absentes mais que le document contient des données
d'entrée (U parois, zone climatique, type bâtiment, surface, systèmes CVC, ventilation, etc.),
tu peux fournir une ESTIMATION APPROXIMATIVE basée sur ton expertise.

Règles d'estimation :
- N'estime que si tu as suffisamment de données d'entrée dans le texte
- Sois conservateur (marge d'erreur ±15%)
- Ajoute le suffixe "_estime" à la clé : ex. "rt2012_bbio_estime": 52.0
- Pour Cep : n'estime que si les systèmes énergétiques (chauffage, ECS, ventilation) sont décrits
- Pour Bbio : possible si tu as les U parois, zone, orientation, surfaces vitrées
- Pour Tic : possible si zone climatique connue (valeur conventionnelle)
...
{{
  "valeurs_extraites": {{}},
  "valeurs_estimees": {{}},
  "donnees_disponibles": "résumé en 1 phrase des données d'entrée trouvées"
}}

Exemple :
{{
  "valeurs_extraites": {{"rt2012_airtightness": 1.7}},
  "valeurs_estimees": {{"rt2012_tic_estime": 26.0, "rt2012_cep_estime": 85.0}},
  "donnees_disponibles": "Zone H1a, U murs 0.24 W/m².K, PAC COP 3.24, ventilation récup 69%, surface 1953 m²"
}}
...
{text[:8000]}"""
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                extraites = response_data.get('valeurs_extraites', {})
                estimees  = response_data.get('valeurs_estimees', {})
                donnees_dispo = response_data.get('donnees_disponibles', '')

                if donnees_dispo:
                    print(f"[parse_pdf] Données détectées : {donnees_dispo}")
                if estimees:
                    print(f"[parse_pdf] Valeurs estimées : {list(estimees.keys())}")

                # Fusionner : extraites en priorité, estimées en complément
                combined = {}
                for k, v in extraites.items():
                    if v is not None:
                        combined[k] = float(v)
                for k, v in estimees.items():
                    if v is not None:
                        combined[k] = float(v)  # clé avec suffixe _estime
    ...
    for pattern, key in [
        ...
        (r'[Pp]erm[eé]abilit[eé]\s*[=:]\s*([\d.,]+)','rt2012_airtightness'),
...
def analyze_document(document, data):
    """
    Applique les valeurs extraites (et estimées) au document.
    Les valeurs extraites (sans suffixe) sont prioritaires.
    Les valeurs estimées (suffixe _estime) ne remplissent que les champs vides.
    """
    def get_val(key):
        """Retourne la valeur extraite si dispo, sinon l'estimée."""
        return data.get(key) or data.get(f'{key}_estime')

    # ── FR RT2012 ──
    document.rt2012_bbio         = get_val('rt2012_bbio')
    document.rt2012_cep          = get_val('rt2012_cep')
    document.rt2012_tic          = get_val('rt2012_tic')
    document.rt2012_airtightness = get_val('rt2012_airtightness')
    document.rt2012_enr          = get_val('rt2012_enr')
    # ── FR RE2020 ──
    document.re2020_energy_efficiency = get_val('re2020_energy_efficiency')
    document.re2020_thermal_comfort   = get_val('re2020_thermal_comfort')
    document.re2020_carbon_emissions  = get_val('re2020_carbon_emissions')
    # ── BE PEB ──
    document.peb_espec      = get_val('peb_espec')
    document.peb_ew         = get_val('peb_ew')
    document.peb_u_mur      = get_val('peb_u_mur')
    document.peb_u_toit     = get_val('peb_u_toit')
    document.peb_u_plancher = get_val('peb_u_plancher')
    # ── CH MINERGIE / SIA380 ──
    document.minergie_qh   = get_val('minergie_qh')
    document.minergie_qtot = get_val('minergie_qtot')
    document.minergie_n50  = get_val('minergie_n50')
    document.sia380_qh     = get_val('sia380_qh') or get_val('minergie_qh')
    # ── CA CNEB ──
    document.cneb_ei           = get_val('cneb_ei')
    document.cneb_u_mur        = get_val('cneb_u_mur')
    document.cneb_u_toit       = get_val('cneb_u_toit')
    document.cneb_u_fenetre    = get_val('cneb_u_fenetre')
    document.cneb_infiltration = get_val('cneb_infiltration')
    # ── LU LENOZ ──
    document.lenoz_ep    = get_val('lenoz_ep')
    document.lenoz_ew    = get_val('lenoz_ew')
    document.lenoz_u_mur = get_val('lenoz_u_mur')
    document.lenoz_u_toit= get_val('lenoz_u_toit')

    # Stocker les clés estimées dans admin_notes si pas déjà renseignées
    estimees = {k: v for k, v in data.items() if k.endswith('_estime')}
    if estimees:
        note_estimee = "⚠️ Valeurs estimées par IA (à vérifier) : " + ", ".join(
            f"{k.replace('_estime','').replace('_',' ')} ≈ {v}" for k, v in estimees.items()
        )
        if document.admin_notes:
            document.admin_notes = note_estimee + "\n\n" + document.admin_notes
        else:
            document.admin_notes = note_estimee
